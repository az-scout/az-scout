...
Your `static/` directory already ships in your wheel, so vendored files behave identically across
local dev, SaaS publishing, and customer self-hosting. To keep things maintainable, mirror the
core and add a dependency-free (stdlib-only) `tools/vendor_assets.py`-style sync script with pinned
versions; the committed files are the source of truth, so **no npm / bundler / build step** is
introduced.
