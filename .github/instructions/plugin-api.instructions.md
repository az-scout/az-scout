...
## Plugin assets (same-origin mount, no CSP enforced)

The core does **not** enforce a Content-Security-Policy, and ships all of its own third-party assets
vendored locally. Plugin static dirs are mounted **same-origin**
(`/plugins/{name}/static`, `/internal/{name}/static`), so plugin-vendored assets are served
alongside the app — and, because there is no CSP, plugins **may** also load assets from an external
CDN if they choose.

- Vendoring extra third-party assets into the plugin's own package is **recommended** (best for
  offline/air-gapped self-hosting and reproducibility) — see `plugin-author.instructions.md` →
  *Third-party / vendored assets*. Loading from a CDN also works but adds a runtime network dependency.
