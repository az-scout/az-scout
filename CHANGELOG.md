...
- **Vendoring third-party assets is now a recommended (optional) convention for plugins** – plugin static dirs are served same-origin, so plugins should (1) reuse the libraries the core already vendors — Bootstrap, Bootstrap Icons, D3, marked, highlight.js, simple-datatables — and (2) prefer vendoring any *additional* dependency into their own `static/vendor/` and referencing it via `/plugins/{name}/static/vendor/…` for offline/air-gapped reproducibility. Because the app enforces no Content-Security-Policy, plugins *may* also load assets from an external CDN if they choose. The plugin authoring guidance (`.github/instructions/plugin-*.instructions.md`, `docs/plugins/index.md`, the `create-plugin` skill) and the plugin scaffold now document this; scaffolded plugins ship a `static/vendor/README.md` explaining the pattern.
- **Removed the restrictive `'self'`-only Content-Security-Policy** – the app no longer sends a CSP header on HTML responses, so third-party plugins can again load their own JS/CSS/fonts from external CDNs (unpkg, cdnjs, jsdelivr, …). The core app still ships its own assets vendored locally, and vendoring remains a recommended (optional) practice for plugins. Note: removing the header also drops the `frame-ancestors 'none'` (clickjacking) and `img-src` hardening it carried; a CDN-friendly / opt-in CSP could be reintroduced later if that hardening is wanted back.
